...
use tabs::convert::{open_gr2_file, open_gts_file, open_lsf_file};
        ...
        if let Event::KeyDown(key_event) = e {
            ...
            if is_cmd_or_ctrl && is_o_key {
                match current_tab {
                    ...
                    // Dialogue tab (6) - no CMD+O action, use toolbar buttons instead
                    7 => {
...
fn tab_bar(active_tab: RwSignal<usize>, convert_subtab: RwSignal<usize>) -> impl IntoView {
    v_stack((
        ...
        h_stack((
            ...
            tab_button("🧪 Dyes", 4, active_tab),
            tab_button("🔍 Search", 5, active_tab),
            tab_button("💬 Dialogue", 6, active_tab),
            tab_button("🛠 Workbench", 7, active_tab),
...
fn tab_content(
    ...
    dyn_container(
        ...
        move |tab_index| match tab_index {
            ...
            .into_any(),
            4 => dyes_tab(app_state.clone(), dyes_state.clone()).into_any(),
            5 => search_tab(
                app_state.clone(),
                search_state.clone(),
                config_state.clone(),
