                ...
                v_stack_from_iter(nodes.into_iter().enumerate().map(|(idx, node)| {
                    ...
                    .on_secondary_click(move |_| {
                        if full_path.exists() {
                            ...
                            if !is_dir {
                                menu =
                                    menu.entry(MenuItem::new("Open in Editor").action(move || {
...
fn scan_directory(
    ...
    for entry in entries {
        ...
        let expanded = if is_dir {
            persisted.get(&full_path).copied().unwrap_or(depth < 2)
        } else {
            false
        };
        ...
        let parent_visible_and_expanded = parent_idx
            .map(|pi| nodes[pi].is_visible.get_untracked() && nodes[pi].is_expanded.get_untracked())
            .unwrap_or(true);
