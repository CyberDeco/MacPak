...
use super::{
    convert, create, extract, gr2, list, loca, mod_cmd, pak, search, texture, virtual_texture,
};
...
impl Gr2Commands {
    pub fn execute(&self) -> anyhow::Result<()> {
        match self {
            Gr2Commands::Inspect { path } => gr2::inspect(path),
            Gr2Commands::Extract { path, output } => gr2::extract_json(path, output),
            Gr2Commands::Decompress { path, output } => gr2::decompress(path, output.as_deref()),
            Gr2Commands::ToGlb { path, output } => gr2::convert_to_glb(path, output.as_deref()),
            Gr2Commands::FromGltf { path, output } => gr2::convert_to_gr2(path, output.as_deref()),
            Gr2Commands::Bundle {
                path,
                output,
                game_data,
...
impl VirtualTextureCommands {
    pub fn execute(&self) -> anyhow::Result<()> {
        match self {
            VirtualTextureCommands::List { path } => virtual_texture::list(path),
            VirtualTextureCommands::Extract {
                ...
                virtual_texture::extract(path, output, texture.as_deref(), &layers, *all_layers)
            }
