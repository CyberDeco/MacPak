...
mod tests {
    ...
    #[test]
    fn test_extract_bc_tile_simple() {
        // Create simple test data: 16x16 texture = 4x4 blocks = 16 blocks
        // Using block_size=16 (BC3/BC5)
        let block_size = 16;
        let blocks = 16; // 4x4
        let src_data: Vec<u8> = (0..blocks).flat_map(|i| vec![i as u8; block_size]).collect();

        // Extract a 2x2 block tile starting at block (0,0)
        let tile = extract_bc_tile(
            &src_data,
            16, // 16 pixels wide = 4 blocks
            0,  // tile_x
            0,  // tile_y
            8,  // 8 pixels = 2 blocks wide
            8,  // 8 pixels = 2 blocks high
            block_size,
        );

        // Should get blocks 0, 1, 4, 5
        assert_eq!(tile.len(), 4 * block_size);
    }

    #[test]
    fn test_extract_bc_tile_with_clamp() {
        // Create simple test data: 16x16 texture = 4x4 blocks = 16 blocks
        let block_size = 16;
        let blocks = 16; // 4x4
        let src_data: Vec<u8> = (0..blocks).flat_map(|i| vec![i as u8; block_size]).collect();

        // Extract a 3x3 block tile that extends beyond the source
        let tile = extract_bc_tile_with_clamp(
            &src_data,
            16, // 16 pixels wide = 4 blocks
            16, // 16 pixels high = 4 blocks
            8,  // tile_x = 2 blocks in
            8,  // tile_y = 2 blocks in
            12, // 12 pixels = 3 blocks wide (will extend beyond)
            12, // 12 pixels = 3 blocks high
            block_size,
        );

        // Should get 3x3 = 9 blocks, with edge blocks clamped
        assert_eq!(tile.len(), 9 * block_size);
    }
}
